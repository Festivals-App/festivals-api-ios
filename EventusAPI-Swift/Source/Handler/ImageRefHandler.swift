...
class ImageRefHandler {
    ...
    func all(images completion: @escaping (_ images: [ImageRef]?, _ error: Error?) -> (Void)) {
        ...
        self.images(with: nil) { festivals, error in
    ...
    ///     - images: The fetched images.
    ///     - error: If the request failed the error can provide more information about the failure reason.
    func images(with objectIDs: [Int]?, completion: @escaping (_ images: [ImageRef]?, _ error: Error?) -> (Void)) {
        
        self.webservice.fetch("image", with: objectIDs, including: nil) { (objects, err) -> (Void) in
