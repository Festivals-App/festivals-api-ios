...
class TagHandler {
    ...
    func all(tags completion: @escaping (_ tags: [Tag]?, _ error: Error?) -> (Void)) {
        ...
        self.tags(with: nil) { tags, error in
    ...
    ///     - tags: The fetched tags.
    ///     - error: If the request failed the error can provide more information about the failure reason.
    func tags(with objectIDs: [Int]?, completion: @escaping (_ tags: [Tag]?, _ error: Error?) -> (Void)) {
        
        self.webservice.fetch("tag", with: objectIDs, including: nil) { (objects, err) -> (Void) in
