...
struct Link: Codable {
    ...
    /*
    func url(mobile: Bool) -> URL {
        switch self.service {
        case .unknown,.websiteURL, .bandcampProfileURL, .bandcampTrackURL, .appleMusicStoreURL, .tripadvisorURL:
            return URL.init(string: self.referrer)!
        case .mail:
            return URL.init(string: "tel:%@")!
        case .phone:
            return URL.init(string: "mailto:%@?subject=''&body=''")!
        case .youtubeUserRef:
            if mobile {
                return URL.init(string: "youtube://www.youtube.com/user/\(self.referrer)")!
            } else {
                return URL.init(string: "https://www.youtube.com/user/\(self.referrer)")!
            }
        case .youtubeChannelRef:
            if mobile {
                return URL.init(string: "youtube://www.youtube.com/channel/\(self.referrer)")!
            } else {
                return URL.init(string: "https://www.youtube.com/channel/\(self.referrer)")!
            }
        case .youtubePlaylistRef:
            if mobile {
                return URL.init(string: "youtube://www.youtube.com/playlist?list=\(self.referrer)")!
            } else {
                return URL.init(string: "https://www.youtube.com/playlist?list=\(self.referrer)")!
            }
        case .youtubeMusicPlaylistRef:
            if mobile {
                return URL.init(string: "youtube://music.youtube.com/playlist?list=\(self.referrer)")!
            } else {
                return URL.init(string: "https://music.youtube.com/playlist?list=\(self.referrer)")!
            }
        case .soundcloudProfileRef:
            if mobile {
                return URL.init(string: "https://soundcloud.com/\(self.referrer)")!
            } else {
                return URL.init(string: "https://soundcloud.com/\(self.referrer)")!
            }
...
class LinkHandler {
    ...
    func all(links completion: @escaping (_ links: [Link]?, _ error: Error?) -> (Void)) {
        ...
        self.links(with: nil) { links, error in
    ...
    ///     - links: The fetched links.
    ///     - error: If the request failed the error can provide more information about the failure reason.
    func links(with objectIDs: [Int]?, completion: @escaping (_ links: [Link]?, _ error: Error?) -> (Void)) {
        
        self.webservice.fetch("link", with: objectIDs, including: nil) { (objects, err) -> (Void) in
