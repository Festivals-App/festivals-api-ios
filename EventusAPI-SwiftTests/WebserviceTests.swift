...
class WebserviceTests: XCTestCase {
    ...
    func testServiceFetchQueryCreation() throws {
        ...
        let allFestivals = self.service.makeFetchQuery(with: "festival", nil, including: nil)
        print(allFestivals)
        
        let festivalWithInclude = self.service.makeFetchQuery(with: "festival", [1], including:["image", "tag"])
        print(festivalWithInclude)
        
        let twoFestivals = self.service.makeFetchQuery(with: "festival", [1,2], including: nil)
        print(twoFestivals)
    }
}
